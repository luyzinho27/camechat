...
const firebaseConfig = {
  apiKey: "AIzaSyBkk35zijG29c6Yw0Zz9afTjhzYaobtV8o",
  authDomain: "cametachat.firebaseapp.com",
  projectId: "cametachat",
  storageBucket: "cametachat.firebasestorage.app",
  messagingSenderId: "268705300",
  appId: "1:268705300:web:54ae982b6855e50be19101"
};

const BACKEND_BASE_URL = (window.CAMETACHAT_BACKEND_URL || window.CAMECHAT_BACKEND_URL || '').replace(/\/$/, '');
const RTC_CONFIG = {
    iceServers: [
...
const ADMIN_TAB_STORAGE_KEY = 'cametachat_admin_tab';
const THEME_STORAGE_KEY = 'cametachat_theme';
const SOUND_NOTIFICATIONS_STORAGE_KEY = 'cametachat_sound_notifications';
const MESSAGE_TONE_STORAGE_KEY = 'cametachat_message_tone';
const CALL_TONE_STORAGE_KEY = 'cametachat_call_tone';
const READ_RECEIPTS_STORAGE_KEY = 'cametachat_read_receipts';
const DESKTOP_NOTIFICATIONS_STORAGE_KEY = 'cametachat_desktop_notifications';
const ONLINE_STATUS_VISIBILITY_STORAGE_KEY = 'cametachat_online_status_visibility';
const CHAT_FONT_SIZE_STORAGE_KEY = 'cametachat_chat_font_size';
const CHAT_BACKGROUND_STORAGE_KEY = 'cametachat_chat_background';
const MEDIA_AUTO_SAVE_STORAGE_KEY = 'cametachat_media_auto_save';
const MEDIA_PC_FOLDER_READY_STORAGE_KEY = 'cametachat_media_pc_folder_ready';
const MEDIA_HANDLE_DB_NAME = 'cametachat_local_media';
...
const MEDIA_DOWNLOAD_SCOPE_QUERY_PARAM = 'cametachat_scope';
const CALL_BLOCK_STORAGE_KEY = 'cametachat_block_incoming_calls';
const LAST_SEEN_VISIBILITY_STORAGE_KEY = 'cametachat_last_seen_visible';
const LANGUAGE_STORAGE_KEY = 'cametachat_language';
const MUTED_FRIENDS_STORAGE_KEY_PREFIX = 'cametachat_muted_friends_';
const FRIENDS_CACHE_STORAGE_KEY_PREFIX = 'cametachat_friends_cache_';
const USERS_CACHE_STORAGE_KEY_PREFIX = 'cametachat_users_cache_';
const USERS_CACHE_MAX_PHOTO_DATA_LENGTH = 30000;
const ANDROID_FCM_TOKEN_STORAGE_KEY = 'cametachat_android_fcm_token';
...
function notifyAndroidAppReady(screen = 'auth') {
    try {
        if (window.CametaChatAndroid && typeof window.CametaChatAndroid.notifyAppReady === 'function') {
            window.CametaChatAndroid.notifyAppReady(String(screen || 'auth'));
        }
    } catch (error) {
...
function isAndroidWebViewRuntime() {
    if (window.CametaChatAndroid) return true;
...
function callAndroidBridgeMethod(methodName, ...args) {
    try {
        if (!window.CametaChatAndroid) return;
        const method = window.CametaChatAndroid[methodName];
        if (typeof method === 'function') {
            const result = method.call(window.CametaChatAndroid, ...args);
...
function buildDeviceDownloadPath(msg, fileName, mimeType = '', options = {}) {
    ...
    return `CametaChat/Media/${scopePrefix}${category}/${safeName}`;
}
...
async function ensurePcMediaFolderStructure(rootHandle) {
    const cametachatDir = await rootHandle.getDirectoryHandle('CametaChat', { create: true });
    const mediaDir = await cametachatDir.getDirectoryHandle('Media', { create: true });
...
async function getPcMediaRootHandle(interactive = false) {
    ...
    if (!handle && interactive) {
        try {
            handle = await window.showDirectoryPicker({ id: 'cametachat-media-root', mode: 'readwrite' });
...
const ABOUT_CONTENT_HTML = `
    <p>CametaChat e uma aplicacao de mensagens em tempo real com suporte a texto, midias e chamadas.</p>
...
function showIncomingDesktopNotification(senderUid, messageData) {
    ...
    const icon = getSafePhotoData(sender?.photoData) || getSafePhotoUrl(sender?.photoURL) || 'images/cametachat_logo.png';
    ...
    try {
        const notification = new Notification(`Nova mensagem de ${senderName}`, {
            body,
            icon,
            tag: `cametachat-message-${senderUid}`,
            renotify: true
        });
            ...
            if (messagesContainer) {
                messagesContainer.innerHTML = `
                    <div class="welcome-message">
                        <img src="images/cametachat_logo.png" alt="Logo do CametaChat" class="welcome-logo">
...
function handleAndroidSharePayload(rawPayload) {
    ...
    if (currentUser) {
        ...
        alert('Faça login para compartilhar no CametaChat.');
    }
}
...
async function handleAndroidCallAction(rawPayload) {
    ...
    if (!currentUser) {
        ...
        alert('Faça login para atender a chamada no CametaChat.');
        return;
    }
...
window.CametaChatApp = window.CametaChatApp || {};
window.CametaChatApp.handleAndroidBackPress = handleAndroidBackPress;
window.CametaChatApp.handleNativeGoogleSignInResult = handleNativeGoogleSignInResult;
window.CametaChatApp.handleAndroidSharePayload = handleAndroidSharePayload;
window.CametaChatApp.handleAndroidShareUploadResult = handleAndroidShareUploadResult;
window.CametaChatApp.handleAndroidCallAction = handleAndroidCallAction;
window.CametaChatApp.handleAndroidMessageOpen = handleAndroidMessageOpen;
window.CametaChatApp.handleAndroidFcmToken = handleAndroidFcmToken;
window.CameChatApp = window.CametaChatApp;
