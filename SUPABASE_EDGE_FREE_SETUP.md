# CametaChat sem Render usando Supabase + Firebase

Este e o caminho mais realista para colocar o CametaChat online de novo sem depender do backend Node no Render.
...
## Limites oficiais que afetam o CametaChat
...
- `supabase/functions/cametachat-api/index.ts`

## Passo 1: criar o projeto no Supabase

1. Crie ou reutilize um projeto no Supabase.
2. Em `Storage`, crie um bucket publico chamado `cametachat-media`.
...
```env
CAMETACHAT_STORAGE_BUCKET=cametachat-media
CAMETACHAT_STORAGE_PUBLIC_BASE_URL=https://SEU_PROJECT_REF.supabase.co/storage/v1/object/public/cametachat-media
...
```bash
supabase functions deploy cametachat-api --no-verify-jwt
```

Base URL da funcao depois do deploy:

```text
https://SEU_PROJECT_REF.supabase.co/functions/v1/cametachat-api
```

## Passo 4: apontar o frontend web

No `index.html`, ajuste `window.CAMETACHAT_BACKEND_URL` para a URL da funcao:

```js
window.CAMETACHAT_BACKEND_URL = 'https://SEU_PROJECT_REF.supabase.co/functions/v1/cametachat-api';
```
...
buildConfigField "String", "BACKEND_URL", "\"https://SEU_PROJECT_REF.supabase.co/functions/v1/cametachat-api\""
```
...
```text
https://SEU_PROJECT_REF.supabase.co/functions/v1/cametachat-api/api/health
```
...
3. confirme que as midias novas estao indo para o bucket `cametachat-media`
