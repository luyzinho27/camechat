...
const FUNCTION_NAME = 'camechat-api'
const FIREBASE_SCOPE = 'https://www.googleapis.com/auth/firebase.messaging'
const PROFILE_FILE_LIMIT_BYTES = 5 * 1024 * 1024
const CHAT_FILE_LIMIT_BYTES = 50 * 1024 * 1024
const STORAGE_BUCKET = ((Deno.env.get('CAMECHAT_STORAGE_BUCKET') || '').trim() || 'camechat-media')
const STORAGE_PUBLIC_BASE_URL = trimTrailingSlash(Deno.env.get('CAMECHAT_STORAGE_PUBLIC_BASE_URL') || '')
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.error('camechat-api error', error)
